...
class contactoModel {
    static async getAll({ motivo, medio } = {}) {
        ...
        let sql = "SELECT id_consulta, nombre, email, telefono, motivo, mensage, medio FROM Consulta";
        const queryParams = [];
    ...
    static async getById({ id }) {
        await initializeConnection();
        const [result] = await connection.query('SELECT * FROM Consulta WHERE id_consulta = ?', [id]);
    ...
    static async delete({ id }) {
        await initializeConnection();
        const [result] = await connection.query('DELETE FROM Consulta WHERE id_consulta = ?', [id]);
